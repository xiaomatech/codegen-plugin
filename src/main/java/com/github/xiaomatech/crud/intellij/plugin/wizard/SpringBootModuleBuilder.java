...
import com.github.xiaomatech.crud.intellij.plugin.ui.CrudConnView;
import com.github.xiaomatech.crud.intellij.plugin.ui.CrudDbView;
import com.github.xiaomatech.crud.intellij.plugin.ui.CrudTableView;
import com.github.xiaomatech.crud.intellij.plugin.util.CrudUtils;
import com.github.xiaomatech.crud.intellij.plugin.util.PsiFileUtils;
import com.github.xiaomatech.crud.intellij.plugin.util.Selection;
...
public class SpringBootModuleBuilder extends ModuleBuilder {
    ...
    private void initProject(Project project, Selection selection) throws Exception {
        ...
        //RedisConfig生成
        PsiFileUtils.createRedisConfig(project, createPackageDir(selection.getPackage() + ".config"), selection);
        //Application类生成
        PsiFileUtils.createApplicationJava(project, createPackageDir(selection.getPackage()), selection);

        PsiFileUtils.createResultJava(project, createPackageDir(selection.getPackage() + ".controller.common"), selection);
	...
	PsiFileUtils.createDomainUtils(project, createTestPackageDir(selection.getPackage()), selection);
        ...
        selection.setModelPackage(selection.getPackage() + ".entity");
        selection.setDaoPackage(selection.getPackage() + ".repository");

        selection.setCommonPackage(selection.getPackage() + ".common");
    ...
    private VirtualFile createTestPackageDir(String packageName) {
